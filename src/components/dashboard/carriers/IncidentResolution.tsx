...
interface IncidentResolutionProps {
  incidentData: Record<string, number>;
}

export default function IncidentResolution({ incidentData }: IncidentResolutionProps) {
  // Convertir los datos a un formato adecuado para el gráfico
  const chartData = Object.entries(incidentData)
    .map(([name, value]) => ({ name, value }))
    .sort((a, b) => a.value - b.value); // Ordenar por tiempo de resolución (ascendente)
  
  return (
    <CardComponent title="Tiempo de Resolución de Incidencias" height={350}>
