...
export async function GET(request: Request) {
  try {
    ...
    console.log(`API Call: Generating data for timeRange: ${timeRange}`);
    
    // Generar datos simulados basados en el rango de tiempo
    const data = generateMockData(timeRange);

    // Agregar un pequeño retraso para simular latencia de red (opcional)
...
function generateMockData(range: string) {
  ...
  const topCustomers = [
    { id: 1, name: 'APLIN', level: 'Nivel 26', guides: Math.floor(guidesGenerated * 0.08), revenue: Math.floor(totalRevenue * 0.09), margin: 40.2, status: 'active', nps: 82, trend: [60, 65, 68, 73, 75, 72, 78] },
    { id: 2, name: 'CHICOS OLÉ', level: 'Nivel 26', guides: Math.floor(guidesGenerated * 0.075), revenue: Math.floor(totalRevenue * 0.08), margin: 38.5, status: 'active', nps: 79, trend: [58, 60, 65, 67, 70, 68, 71] },
    { id: 3, name: 'DESIGUALEX', level: 'Nivel 10', guides: Math.floor(guidesGenerated * 0.065), revenue: Math.floor(totalRevenue * 0.07), margin: 35.8, status: 'active', nps: 76, trend: [52, 55, 58, 62, 60, 65, 67] },
    { id: 4, name: 'CLAROSHOP', level: 'Nivel 26', guides: Math.floor(guidesGenerated * 0.06), revenue: Math.floor(totalRevenue * 0.065), margin: 37.9, status: 'active', nps: 81, trend: [55, 58, 62, 64, 67, 70, 73] },
    { id: 5, name: 'EMISSARY', level: 'Nivel 10', guides: Math.floor(guidesGenerated * 0.055), revenue: Math.floor(totalRevenue * 0.06), margin: 32.7, status: 'active', nps: 72, trend: [48, 50, 52, 54, 53, 56, 58] },
    { id: 6, name: 'TECH GADGETS', level: 'Nivel 10', guides: Math.floor(guidesGenerated * 0.05), revenue: Math.floor(totalRevenue * 0.055), margin: 31.2, status: 'at_risk', nps: 68, trend: [46, 49, 51, 49, 52, 50, 51] },
    { id: 7, name: 'ZAPATERIA LÓPEZ', level: 'Nivel 1', guides: Math.floor(guidesGenerated * 0.045), revenue: Math.floor(totalRevenue * 0.05), margin: 28.5, status: 'active', nps: 73, trend: [42, 45, 48, 50, 49, 51, 53] },
    { id: 8, name: 'SANBORNS', level: 'Nivel 26', guides: Math.floor(guidesGenerated * 0.04), revenue: Math.floor(totalRevenue * 0.045), margin: 36.8, status: 'active', nps: 78, trend: [51, 54, 57, 60, 62, 61, 64] },
    { id: 9, name: 'PURPLIFY', level: 'Nivel 1', guides: Math.floor(guidesGenerated * 0.035), revenue: Math.floor(totalRevenue * 0.04), margin: 27.1, status: 'inactive', nps: 65, trend: [40, 42, 44, 41, 43, 45, 42] },
    { id: 10, name: 'BOUTIQUE AMPM', level: 'Nivel 10', guides: Math.floor(guidesGenerated * 0.03), revenue: Math.floor(totalRevenue * 0.035), margin: 30.4, status: 'active', nps: 70, trend: [44, 46, 49, 51, 50, 53, 55] }
  ];
